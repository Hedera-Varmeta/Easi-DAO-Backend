...
import {Address, Config, CurrencyConfig, KmsDataKey} from '../../database/entities';
...
export class AddressesService {
    ...
    async updateMinimum(key: string, value: string): Promise<any> {
        ...
        var data = await this.configRepository.findOne({
            where: {key: key}
        });
    ...
    async getAddress(currency: CurrencyConfig): Promise<Address> {
        ...
        try {
            if (!currency) {
                throw ApiCauses.WALLET_WITH_CURRENCY_NOT_CREATED;
            }
    ...
    async generateOneWalletAddress(currency: CurrencyConfig, dataKey: KmsDataKey) {
        ...
        if (privateKeyHandled.length > 250) {
            throw ApiCauses.ENCRYPT_PRIVATE_KEY_ERROR;
        }
